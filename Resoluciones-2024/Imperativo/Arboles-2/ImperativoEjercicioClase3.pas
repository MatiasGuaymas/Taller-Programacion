...
procedure buscarSocioMasChico(a: arbol; var s: socio);
begin
  if a <> nil then
  begin
    if a^.HI = nil then
...
begin
  if a = nil then
    ...
    ExisteSocio := ExisteSocio(a^.HI, num) or ExisteSocio(a^.HD, num);
end;

function CantidadSociosEntre(a: arbol; num1, num2: integer): integer;
begin
  if a = nil then
    CantidadSociosEntre := 0
  else
  begin
    if (a^.dato.numero >= num1) and (a^.dato.numero <= num2) then
      CantidadSociosEntre := 1 + CantidadSociosEntre(a^.HI, num1, num2) + CantidadSociosEntre(a^.HD, num1, num2)
    else if a^.dato.numero < num1 then
      CantidadSociosEntre := CantidadSociosEntre(a^.HD, num1, num2)
    else
      CantidadSociosEntre := CantidadSociosEntre(a^.HI, num1, num2);
  end;
end;
...
begin
  ...
  writeln('Numero de socio mas grande: ', NumeroSocioMasGrande(a));
  buscarSocioMasChico(a, s);
  writeln('Socio con numero de socio mas chico: ', s.numero, ' ', s.nombre, ' ', s.edad);
  writeln('Ingrese un numero de socio a buscar: ');
  readln(num);
  if ExisteSocio(a, num) then
    writeln('El numero de socio ', num, ' existe en el arbol')
  else
    writeln('El numero de socio ', num, ' no existe en el arbol');
  writeln('Ingrese dos numeros de socio para buscar la cantidad de socios entre ellos: ');
  readln(num1);
  readln(num2);
  writeln('Cantidad de socios entre ', num1, ' y ', num2, ': ', CantidadSociosEntre(a, num1, num2));
end.
